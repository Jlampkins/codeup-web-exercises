...
describe('padArray', function() {
    ...
    it('should pass the example problems in the readme', function() {
        expect(padArray([1, 2, 3], 5, 0)).toEqual([1, 2, 3, 0, 0])
        expect(padArray([1, 2, 3], 2, 0)).toEqual([1, 2, 3])
        expect(padArray([1, 2, 3], 5, 'a')).toEqual([1, 2, 3, 'a', 'a'])
    });
});
...
setTimeout(function() {
    document.querySelector("body > div > div.alert > span.menu.bar.failure-list > a").click();
}, 100);
